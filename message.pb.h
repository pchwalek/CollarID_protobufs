...
/* Enum definitions */
/* Outcome of a config transaction (remote schedule push). Uplinked once
 after CMD_CONFIG_COMMIT is processed — the server's only visibility into
 whether a push landed; without it, NACKs died in METADATA.CSV. */
typedef enum config_ack_status {
    CONFIG_ACK_STATUS_CFG_ACK_NONE = 0,
    CONFIG_ACK_STATUS_CFG_ACK_APPLIED = 1, /* staged config applied + persisted */
    CONFIG_ACK_STATUS_CFG_ACK_MISSING_FRAGS = 2, /* commit rejected; see cfg_missing_mask */
    CONFIG_ACK_STATUS_CFG_ACK_RAILS_REJECTED = 3, /* schedule would silence the device (floor/radio rails) */
    CONFIG_ACK_STATUS_CFG_ACK_NO_TXN = 4, /* commit with no open transaction */
    CONFIG_ACK_STATUS_CFG_ACK_EXPIRED = 5 /* staging expired before commit arrived */
} config_ack_status_t;
...
typedef struct ack_packet {
    bool has_cfg_txn_id;
    uint32_t cfg_txn_id; /* echoes DownlinkPacket.cfg_txn_id */
    bool has_cfg_status;
    config_ack_status_t cfg_status;
    /* For MISSING_FRAGS: bit i = fragment_index i missing.
 For RAILS_REJECTED (fw 301+): the SCHED_RAIL_* code of the refusing
 rule (1 slot-count, 2 grown-slot-bare, 3 radio-off, 4 cadence-floor,
 5 both-radios, 6 window-overlap). 0 = firmware predating the field. */
    bool has_cfg_missing_mask;
    uint32_t cfg_missing_mask;
    bool has_sched_crc;
    uint32_t sched_crc; /* schedule identity AFTER the verdict */
} ack_packet_t;
...
#define _CONFIG_ACK_STATUS_MIN CONFIG_ACK_STATUS_CFG_ACK_NONE
#define _CONFIG_ACK_STATUS_MAX CONFIG_ACK_STATUS_CFG_ACK_EXPIRED
#define _CONFIG_ACK_STATUS_ARRAYSIZE ((config_ack_status_t)(CONFIG_ACK_STATUS_CFG_ACK_EXPIRED+1))
...
#define ack_packet_t_cfg_status_ENUMTYPE config_ack_status_t
...
#define ACK_PACKET_INIT_DEFAULT                  {false, 0, false, _CONFIG_ACK_STATUS_MIN, false, 0, false, 0}
...
#define ACK_PACKET_INIT_ZERO                     {false, 0, false, _CONFIG_ACK_STATUS_MIN, false, 0, false, 0}
...
#define ACK_PACKET_CFG_TXN_ID_TAG                1
#define ACK_PACKET_CFG_STATUS_TAG                2
#define ACK_PACKET_CFG_MISSING_MASK_TAG          3
#define ACK_PACKET_SCHED_CRC_TAG                 4
...
#define ADDON_REPORT_PENDING_CMD_PARAM_TAG       10
...
X(a, STATIC,   OPTIONAL, UINT32,   cfg_txn_id,        1) \
X(a, STATIC,   OPTIONAL, UENUM,    cfg_status,        2) \
X(a, STATIC,   OPTIONAL, UINT32,   cfg_missing_mask,   3) \
X(a, STATIC,   OPTIONAL, UINT32,   sched_crc,         4)
